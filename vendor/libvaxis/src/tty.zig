...
/// Simple buffered writer backed by a raw POSIX fd and std.Io.Writer interface
const BufFdWriter = struct {
    fd: posix.fd_t,
    interface: std.Io.Writer,
    buffer: []u8,

    fn init(fd: posix.fd_t, buffer: []u8) BufFdWriter {
        return .{
            .fd = fd,
            .buffer = buffer,
            .interface = .{
    ...
    fn drain(w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
        const self: *BufFdWriter = @fieldParentPtr("interface", w);
        writeAllInternal(self, data, splat) catch |err| switch (err) {
            error.WriteFailed => return 0,
            else => return err,
        };
        // Count all buffer bytes as consumed
        const from_buf = self.interface.end;
        self.interface.end = 0;
        // Count data bytes written
        var total: usize = 0;
        for (data) |d| total += d.len;
        if (splat > 0 and data.len > 0) {
            total += data[data.len - 1].len * (splat - 1);
        }
        return from_buf + total;
    }

    fn sendFile(w: *std.Io.Writer, reader: *std.Io.File.Reader, limit: std.Io.Limit) std.Io.FileError!usize {
        const self: *BufFdWriter = @fieldParentPtr("interface", w);
        // Flush buffer first
        try flush(w);
        // Then send from file
        // Simple implementation: read into a temp buffer and write
        var total: usize = 0;
        var tmp_buf: [4096]u8 = undefined;
        while (true) {
            const cap: u64 = std.math.cast(u64, limit.bytesLeft()) orelse break;
            const to_read = @as(u64, @intCast(@min(@as(u64, @intCast(tmp_buf.len)), cap)));
            if (to_read == 0) break;
            const n = try reader.doRead(tmp_buf[0..@as(usize, @intCast(to_read))]);
            if (n == 0) break;
            try doWrite(self.fd, tmp_buf[0..n]);
            total += n;
        }
        return total;
    }

    fn flush(w: *std.Io.Writer) std.Io.Error!void {
        const self: *BufFdWriter = @fieldParentPtr("interface", w);
        if (self.interface.end > 0) {
            try doWrite(self.fd, self.buffer[0..self.interface.end]);
            self.interface.end = 0;
        }
    }

    fn writeAllInternal(self: *BufFdWriter, data: []const []const u8, splat: usize) std.posix.WriteError!void {
        if (self.interface.end > 0) {
            try doWrite(self.fd, self.buffer[0..self.interface.end]);
            self.interface.end = 0;
        }
        for (data) |d| try doWrite(self.fd, d);
        for (1..splat) |_| {
            if (data.len > 0) try doWrite(self.fd, data[data.len - 1]);
        }
    }

    fn doWrite(fd: posix.fd_t, data: []const u8) std.posix.WriteError!void {
        var rem = data;
        while (rem.len > 0) {
            const n = posix.write(fd, rem) catch |err| switch (err) {
                error.FileBusy => 0,
                error.WouldBlock => 0,
                else => return err,
            };
...
pub const PosixTty = struct {
    ...
    pub fn init(buf: []u8) !PosixTty {
        ...
        var act = posix.Sigaction{
            .handler = .{ .handler = @ptrCast(&handleWinch) },
        ...
        const buffered_writer = BufFdWriter.init(fd, buf);
    ...
    pub fn writer(self: *PosixTty) *std.Io.Writer {
        return &self.buffered_writer.interface;
    }
    ...
    const SigIntType = @typeInfo(@TypeOf(posix.SIG.WINCH)).@"enum".tag_type;
    fn handleWinch(_: SigIntType) callconv(.c) void {
        handler_mutex.lock();
        defer handler_mutex.unlock();
        var i: usize = 0;
