...
- [x] `read_active_task` 能正确返回当前最高优先级待办任务
- [x] `update_task_log` 能原子性地在任务文件追加日志
- [x] `sync_spec_with_code` 能检测 Spec 和代码之间的明显不一致
- [x] 3 个工具均已注册到 Agent 的工具表中
...
- `2026-04-06` — 开始 T-121 实现，状态 `todo` → `implement`
- `2026-04-06` — 创建 `src/agent/tools/document_tools.zig`，实现 3 个工具
- `2026-04-06` — 在 `src/cli/root.zig` 注册 3 个新工具到 Agent 工具表
- `2026-04-06` — 修复 Zig 0.16 API 编译错误：`std.ArrayList(.empty)`、`std.time` 替换、`std.json.Stringify.valueAlloc` 等
- `2026-04-06` — 修复 `src/utils/fs_helper.zig` 中的 `io.interface`、`rename` 参数等编译错误
- `2026-04-06` — `make build` 和 `make test` 全部通过
- `2026-04-06` — 完成实现，状态 `implement` → `verify`
